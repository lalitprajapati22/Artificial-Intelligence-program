...
d	-- Amount to be measured */ int pour(int fromCap, int toCap, int d)
...
//	Pour "temp" litres from "from" to "to" to += temp;
from -= temp;
...
//	If first jug becomes empty, fill it if (from == 0)
{
...
//	If second jug becomes full, empty it if (to == toCap)
{
...
//	To make sure that m is smaller than n if (m > n)
swap(m, n);

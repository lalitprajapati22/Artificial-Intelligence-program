...
//	stores blank tile coordinates int x, y;
...
//	set number of misplaced tiles node->cost = INT_MAX;

//	set number of moves so far node->level = level;

//	update new blank tile cordinates node->x = newX;
...
//	botton, left, top, right int row[] = { 1, 0, -1, 0 }; int col[] = { 0, -1, 0, 1 };
...
printPath(root->parent); printMatrix(root->mat);
...
//	finally deletes it from the list. while (!pq.empty())
{
...
//	Add child to list of live nodes pq.push(child);
}
}
